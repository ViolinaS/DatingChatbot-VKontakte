...
def get_user_data(user_id):
  user_data = bot_1.users.get(user_ids=user_id, fields="city, sex, bdate, relation")
  return user_data
...
def get_city_from_data():
  ...
  if "city" not in city_data[0].keys():
    ...
    get_country = [country['id'] for country in user_country_search['items']]
    user_country_id = get_country[0]
    user_city_check = bot_2.database.getCities(country_id=user_country_id, q=user_city_title, need_all=0, count=1)
...
def get_bdate_from_data(): 
  ...
  if "bdate" not in user_bdate[0].keys() or len(user_bdate[0]["bdate"]) < 9:
    ...
    while True:
      send_message(user_id, f'Укажите дату вашего рождения в формате D.M.YYYY: ?')
      for event in longpoll_1.listen():
        if event.type == VkEventType.MESSAGE_NEW and event.to_me and user_id == event.user_id:
...
def get_user_relation_data():
  ...
  if user_relation in [0, 8, 7, 4, 3, 2]:
    send_message(user_id, f"Укажите семейное положение (личный статус) искомого человека на клавиатуре", keyboard=RELATION_KEYBOARD)
    for event in longpoll_1.listen():
      if event.type == VkEventType.MESSAGE_NEW and event.to_me and user_id == event.user_id:
    ...
    while user_relation not in RELATION_DATA_RULE:
      send_message(user_id, f"Неверный выбор, выберите личный статус искомого человека на клавиатуре еще раз", keyboard=RELATION_KEYBOARD)
      for event in longpoll_1.listen():
        if event.type == VkEventType.MESSAGE_NEW and event.to_me and user_id == event.user_id:
          user_relation = event.text.lower()
          break
...
def get_random_babe_name(babe_id):
  global babe_info
  babe_info = bot_1.users.get(user_ids=babe_id, fields="photo_max_orig")
...
def get_wanted_3_photos(babe_id):
  babe_photo_album = bot_2.photos.get(owner_id=babe_id, album_id="profile", extended=1) 
  ...
  best3_photos = sorted(best_likes_photo_dict.items(), key=lambda x: x[1], reverse=True)[0:3]
  return best3_photos
...
def prepare_tostart_show():
  babe_for_show = Wanted(babe_id=babe_id, babe_name=babe_name, babe_surname=babe_surname)
  ...
  while True: # Крутим бота в бесконечном цикле
    ...
    if yes_no == "START":
      ...
      while True:
        ...
        if session.query(Wanted_user.id_wanted_user, Black_list.id_notwanted_user).filter(Wanted_user.user_id == user_id, Black_list.user_id == user_id):
